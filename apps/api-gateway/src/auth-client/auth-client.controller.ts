...
export class AuthClientController {
  ...
  async login(@Body() loginData: AuthLoginDto) {
    try {
      ...
      this.logger.error(`Login failed for user: ${loginData.email}`, error.stack);
      throw error;
    }
  }

  @Post('registerSuperAdmin')
  async registerSuperAdmin(@Body() registerData: { email: string; password: string; tenantId: string; }, @Res() res) {
    try {
      this.logger.debug(`Attempting to register superadmin with email: ${registerData.email}`);
      const response = await this.authClientService.registerSuperAdmin(registerData);
      this.logger.debug(`Superadmin registered with email: ${registerData.email}`);
      return res.status(HttpStatus.CREATED).json(response);
    } catch (error) {
      this.logger.error(`Failed to register superadmin with email: ${registerData.email}`, error.stack);
  ...
  async registerResident(@Body() registerData: RegisterResidentDto) {
    try {
      ...
      const response = await this.authClientService.registerResident(registerData);
