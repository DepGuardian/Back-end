...
import { Apartment, ApartmentDocument, ApartmentSchema } from './schemas/apartment.schema';
...
export class ApartmentService implements OnModuleInit {
  ...
  async createApartment(registerData: CreateApartmentDto) {
    this.logger.debug(`Attempting to createApartment with : ${registerData.apartment} for tenant: ${registerData.tenantId}`);
    ...
    try {
      ...
      const newApartment = new ApartmentModel({
        ...
        owner: registerData.owner ? new mongoose.Types.ObjectId(registerData.owner) : null,
      ...
      this.logger.debug(`Successfully registered apartment with number: ${registerData.apartment}`);

      // Retornar apartment
      return savedApartment;

    } catch (error) {
      this.logger.error(`Error registering apartment: ${error.message}`, error.stack);
